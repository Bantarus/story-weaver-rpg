...
-   **State Management:** React Context API
    -   `GameContext` (`src/context/GameContext.tsx`): Manages global game state, including user inputs for story/character creation, generated game data, current game progress (scene, history, inventory, status, alignment), persistence to `localStorage`, and management of saved adventures and character profiles.
...
1.  **User Input (`/create` page):** The user provides story text, character details, and optional advanced generation parameters (tone, length, themes). They can also load a saved character.
...
3.  **AI Flow Invocation:**
    ...
    -   `generateNarrativeOutline`: Creates a narrative outline based on the story, character, and advanced parameters.
    -   `formatGameDataJson`: Transforms the narrative outline into a structured JSON `GameData` object, including scenes, choices, effects, and alignment shifts.
4.  **State Update:** The generated `GameData` is stored in `GameContext`. The user can save this adventure to their library or export it as JSON.
...
6.  **Persistence:** `GameContext` handles saving relevant creation and game state (including inventory, status, alignment, game history, saved adventures, saved characters) to `localStorage`.
7.  **Playthrough Story Generation:** At the end of an adventure, the user can trigger the `generatePlaythroughStory` flow, which uses the game history and context to create a narrative of their specific playthrough.
...
-   Export and Import of game data (JSON format).
