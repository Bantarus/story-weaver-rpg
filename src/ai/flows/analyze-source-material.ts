 ...
 * - AnalyzeSourceMaterialOutput - Output type for the function.
 */
...
const analyzeSourceMaterialPrompt = ai.definePrompt({
...
const analyzeSourceMaterialFlow = ai.defineFlow(
  ...
  async input => {
    const {output} = await analyzeSourceMaterialPrompt(input);
    return output!;
  }
);
